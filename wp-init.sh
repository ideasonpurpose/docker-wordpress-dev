...
if [[ -f /usr/src/site/package.json ]]; then
    ...
    jq -n '{
            ...
            contributors: [],
            "version-everything": {
                files: ["README.md", "wp-content/themes/'${NAME}'/style.css"]
...
jq -s '.[0] * (.[1] | {config, scripts, devDependencies, prettier}) + {
            "version-everything": {
                files: [.[]["version-everything"].files] | flatten | unique
            }}' /tmp/package.json /usr/src/boilerplate-package.json | cat >/usr/src/site/package.json
