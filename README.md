...
The project builds on the official WordPress docker image, currently **[v5.9.3](https://hub.docker.com/_/wordpress)**
...
```
docker run --rm -it -v ${PWD}:/usr/src/site ideasonpurpose/wordpress:0.10.4 init
npm run bootstrap
```
...
```
docker run --rm -it -v %cd%:/usr/src/site ideasonpurpose/wordpress:0.10.4 init
npm run bootstrap
```
